...
<section class="space-y-6">
    <x-card>
        <x-slot name="header">
            ...
            <x-slot
                name="description">{{ __('Once your account is deleted, all of its resources and data will be permanently deleted. Before deleting your account, please download any data or information that you wish to retain.') }}</x-slot>
        </x-slot>
        <x-slot name="content">
            <x-button color='danger' x-data=""
            ...
            <x-modal name="confirm-user-deletion" :show="$errors->isNotEmpty()" focusable>
                <form wire:submit="deleteUser" class="p-6">
                    ...
                    <div class="flex justify-end mt-6">
                        <x-button color='white' x-on:click="$dispatch('close')">
                            {{ __('Cancel') }}
                        </x-button>

                        <x-button color='danger' class="ms-3">
                            {{ __('Delete Account') }}
                        </x-button>
                    </div>
                </form>
